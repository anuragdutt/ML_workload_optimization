...
def get_gpu_info():
	g0 = open("/sys/devices/gpu.0/devfreq/17000000.ga10b/cur_freq")
	for l in g0:
		gf0 = l.split('\n')[0]
	return gf0
...
def logging(i2c_folder, cpu_usage_file, gpu_usage_file, f_stats, stop_logging):
	...
	try:
		...
		while True:
			f_INP_voltage = open(i2c_folder+"in1_input","r")
			f_INP_current = open(i2c_folder+"curr1_input","r")
			...
			voltage = int(f_INP_voltage.read())
			current = int(f_INP_current.read())
...
if __name__ == "__main__":
	...
	i2c_folder = "/sys/bus/i2c/drivers/ina3221/1-0040/hwmon/hwmon4/"
...
# subprocess.Popen("sudo /usr/sbin/nvpmodel -m {}".format(sys.argv[3]),stdin=subprocess.PIPE,shell=True)
	...
	if "batch" in sys.argv[4]:
				command = """sudo -E PYTHONPATH=$PYTHONPATH:/usr/local/lib/python3.8/site-packages python3 lib/{} {}""".format(sys.argv[4],sys.argv[3])
	else:
